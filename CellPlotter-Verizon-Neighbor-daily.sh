...
cat << EOF > /var/www/Verizon-Neighbor-daily-tmp.html
  ...
  <body>
    ...
    <script>
      ...
      L.tileLayer('https://api.maptiler.com/maps/streets/{z}/{x}/{y}.png?key=S3P9GzyoztmyHTfFGU2a',{
        ...
        zoomOffset: -1,
        minZoom: 1,
        Verizonribution: "\u003ca href=\"https://www.maptiler.com/copyright/\" target=\"_blank\"\u003e\u0026copy; MapTiler\u003c/a\u003e \u003ca href=\"https://www.openstreetmap.org/copyright\" target=\"_blank\"\u003e\u0026copy; OpenStreetMap contributors\u003c/a\u003e",
        crossOrigin: true
      }).addTo(map);
...
NeighborCount=`grep $timestamp /var/www/logfiles/Verizon*|grep $CurrentDate|head -1|awk -F "\t" '{print $89"\n"$99"\n"$109"\n"$119"\n"$129"\n"$139"\n"$149"\n"$159"\n"$169"\n"$179"\n"$189"\n"$199"\n"$209"\n"$219"\n"$229"\n"$239"\n"$249}'|grep -e 1 -e 2 -e 3 -e 4 -e 5 -e 6 -e 7 -e 8 -e 9 |wc -l`;
...
grep -v "{icon: }" /var/www/Verizon-Neighbor-daily-tmp.html > /var/www/Verizon-Neighbor-daily.html;
