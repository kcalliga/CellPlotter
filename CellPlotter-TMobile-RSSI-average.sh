...
RoundedLongitude=`printf '%.*f\n' 3 $Longitude|sed 's/-//g'`;
    for rssi in `cat /var/www/logfiles/Google*|grep $CurrentDate|grep -E $RoundedLatitude |grep -E $RoundedLongitude|awk -F "\t" '{print $18}'`; do
...
grep -v "{icon: }" /var/www/TMobile-RSSI-average-tmp.html  >> /var/www/TMobile-RSSI-average.html
