...
cat << EOF > /var/www/TMobile-Finder-tmp.html
...
RSSI=`grep $timestamp /var/www/logfiles/Google*|awk -F '\t' '{print $18}'|head -1|grep -e "-50" -e "-51"`;
...
grep -v "{icon: None" /var/www/TMobile-Finder-tmp.html > /var/www/TMobile-Finder.html;
