...
echo > /tmp/Verizon-RSRP-average-tmp.txt;
...
cat << EOF > /var/www/Verizon-RSRP-average-tmp.html
...
RoundedLongitude=`printf '%.*f\n' 3 $Longitude|sed 's/-//g'`;
    for rsrp in `cat /var/www/logfiles/Verizon*|grep $CurrentDate|grep -E $RoundedLatitude |grep -E $RoundedLongitude|awk -F "\t" '{print $14}'`; do
...
echo "L.marker([$RoundedLatitude, "-"$RoundedLongitude], {icon: $Icon}).addTo(map);" >> /tmp/Verizon-RSRP-average-tmp.txt;

done;

cat /tmp/Verizon-RSRP-average-tmp.txt|sort|uniq >> /var/www/Verizon-RSRP-average-tmp.html;
...
grep -v "{icon: }" /var/www/Verizon-RSRP-average-tmp.html  >> /var/www/Verizon-RSRP-average.html
