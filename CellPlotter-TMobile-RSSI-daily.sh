...
echo > /tmp/TMobile-RSSI-daily-tmp.txt;
echo > /var/www/TMobile-RSSI-daily.html;
...
cat << EOF > /var/www/TMobile-RSSI-daily-tmp.html
...
RSSI=`grep $timestamp /var/www/logfiles/Google*|grep $CurrentDate|awk -F '\t' '{print $18}'|head -1`;
...
echo "L.marker([$Latitude, $Longitude], {icon: $Icon}).addTo(map);" >> /tmp/TMobile-RSSI-daily-tmp.txt;

done;

cat /tmp/TMobile-RSSI-daily-tmp.txt|sort|uniq >> /var/www/TMobile-RSSI-daily-tmp.html;
...
grep -v "{icon: }" /var/www/TMobile-RSSI-daily-tmp.html  >> /var/www/TMobile-RSSI-daily.html
