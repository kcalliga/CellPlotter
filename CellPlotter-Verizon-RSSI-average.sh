...
cat << EOF > /var/www/Verizon-RSSI-average-tmp.html
...
echo "L.marker([$RoundedLatitude, "-"$RoundedLongitude], {icon: $Icon}).addTo(map);" >> /tmp/Verizon-RSSI-average-tmp.txt;

done;

cat /tmp/Verizon-RSSI-average-tmp.txt|sort|uniq >> /var/www/Verizon-RSSI-average-tmp.html;
