...
cat << EOF > /var/www/TMobile-Neighbor-tmp.html
...
NeighborCount=`grep $timestamp /var/www/logfiles/Google*|head -1|awk -F "\t" '{print $89"\n"$99"\n"$109"\n"$119"\n"$129"\n"$139"\n"$149"\n"$159"\n"$169"\n"$179"\n"$189"\n"$199"\n"$209"\n"$219"\n"$229"\n"$239"\n"$249}'|grep -e 1 -e 2 -e 3 -e 4 -e 5 -e 6 -e 7 -e 8 -e 9 |wc -l`;
...
grep -v "{icon: }" /var/www/TMobile-Neighbor-tmp.html > /var/www/TMobile-Neighbor.html;
