...
echo > /tmp/ATT-RSRP-average-tmp.txt;
...
cat << EOF > /var/www/ATT-RSRP-average-tmp.html
...
RoundedLongitude=`printf '%.*f\n' 3 $Longitude|sed 's/-//g'`;
    for rsrp in `cat /var/www/logfiles/ATT*|grep $CurrentDate|grep -E $RoundedLatitude |grep -E $RoundedLongitude|awk -F "\t" '{print $14}'`; do
...
echo "L.marker([$RoundedLatitude, "-"$RoundedLongitude], {icon: $Icon}).addTo(map);" >> /tmp/ATT-RSRP-average-tmp.txt;

done;

cat /tmp/ATT-RSRP-average-tmp.txt|sort|uniq >> /var/www/ATT-RSRP-average-tmp.html;
...
grep -v "{icon: }" /var/www/ATT-RSRP-average-tmp.html  >> /var/www/ATT-RSRP-average.html
