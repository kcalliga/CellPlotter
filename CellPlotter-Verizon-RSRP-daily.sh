...
echo > /tmp/Verizon-RSRP-daily-tmp.txt;
...
cat << EOF > /var/www/Verizon-RSRP-daily-tmp.html
...
RSRP=`grep $timestamp /var/www/logfiles/Verizon*|grep $CurrentDate|awk -F '\t' '{print $14}'|head -1`;
...
echo "L.marker([$Latitude, $Longitude], {icon: $Icon}).addTo(map);" >> /tmp/Verizon-RSRP-daily-tmp.txt;

done;

cat /tmp/Verizon-RSRP-daily-tmp.txt|sort|uniq >> /var/www/Verizon-RSRP-daily-tmp.html;
...
grep -v "{icon: }" /var/www/Verizon-RSRP-daily-tmp.html  >> /var/www/Verizon-RSRP-daily.html
