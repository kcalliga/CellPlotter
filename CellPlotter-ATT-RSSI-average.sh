...
RoundedLongitude=`printf '%.*f\n' 3 $Longitude|sed 's/-//g'`;
    for rssi in `cat /var/www/logfiles/ATT*|grep -E $RoundedLatitude |grep -E $RoundedLongitude|awk -F "\t" '{print $18}'`; do
...
mv /var/www/ATT-RSSI-average-tmp.html /var/www/ATT-RSSI-average.html
