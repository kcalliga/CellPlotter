...
echo > /tmp/ATT-RSSI-average-tmp.txt;
...
cat << EOF > /var/www/ATT-RSSI-average-tmp.html
...
RoundedLongitude=`printf '%.*f\n' 3 $Longitude|sed 's/-//g'`;
    for rssi in `cat /var/www/logfiles/ATT*|grep $CurrentDate|grep -E $RoundedLatitude |grep -E $RoundedLongitude|awk -F "\t" '{print $18}'`; do
...
echo "L.marker([$RoundedLatitude, "-"$RoundedLongitude], {icon: $Icon}).addTo(map);" >> /tmp/ATT-RSSI-average-tmp.txt;

done;

cat /tmp/ATT-RSSI-average-tmp.txt|sort|uniq >> /var/www/ATT-RSSI-average-tmp.html;
...
grep -v "{icon: }" /var/www/ATT-RSSI-average-tmp.html  >> /var/www/ATT-RSSI-average.html
