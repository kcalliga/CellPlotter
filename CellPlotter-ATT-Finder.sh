...
cat << EOF > /var/www/ATT-Finder-tmp.html
...
RSSI=`grep $timestamp /var/www/logfiles/ATT*|awk -F '\t' '{print $18}'|head -1|grep -e "-50" -e "-51"`;
...
grep -v "{icon: None" /var/www/ATT-Finder-tmp.html > /var/www/ATT-Finder.html;
