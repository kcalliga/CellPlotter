...
echo > /tmp/TMobile-RSRP-daily-tmp.txt;
...
cat << EOF > /var/www/TMobile-RSRP-daily-tmp.html
...
RSRP=`grep $timestamp /var/www/logfiles/Google*|grep $CurrentDate|awk -F '\t' '{print $14}'|head -1`;
...
echo "L.marker([$Latitude, $Longitude], {icon: $Icon}).addTo(map);" >> /tmp/TMobile-RSRP-daily-tmp.txt;

done;

cat /tmp/TMobile-RSRP-daily-tmp.txt|sort|uniq >> /var/www/TMobile-RSRP-daily-tmp.html;
...
grep -v "{icon: }" /var/www/TMobile-RSRP-daily-tmp.html  >> /var/www/TMobile-RSRP-daily.html
