...
echo > /tmp/ATT-RSRQ-average-tmp.txt;
...
cat << EOF > /var/www/ATT-RSRQ-average-tmp.html
...
RoundedLongitude=`printf '%.*f\n' 3 $Longitude|sed 's/-//g'`;
    for rsrq in `cat /var/www/logfiles/ATT*|grep $CurrentDate|grep -E $RoundedLatitude |grep -E $RoundedLongitude|awk -F "\t" '{print $15}'|sort|uniq|sed 's/-//g'`; do
...
echo "L.marker([$RoundedLatitude, "-"$RoundedLongitude], {icon: $Icon}).addTo(map);" >> /tmp/ATT-RSRQ-average-tmp.txt;

done;

cat /tmp/ATT-RSRQ-average-tmp.txt|sort|uniq >> /var/www/ATT-RSRQ-average-tmp.html;
...
mv /var/www/ATT-RSRQ-average-tmp.html /var/www/ATT-RSRQ-average.html
