...
echo > /tmp/Verizon-RSSI-daily-tmp.txt;
echo > /var/www/Verizon-RSSI-daily.html;
...
cat << EOF > /var/www/Verizon-RSSI-daily-tmp.html
...
for timestamp in `cat /var/www/logfiles/Verizon*|grep $CurrentDate|grep -v Timestamp|awk -F '\t' '{print $1}'`; do
RSSI=`grep $timestamp /var/www/logfiles/Verizon*|grep $CurrentDate|awk -F '\t' '{print $18}'|head -1`;
Latitude=`grep $timestamp /var/www/logfiles/Verizon*|grep $CurrentDate|awk -F '\t' '{print $3}'|head -1`;
Longitude=`grep $timestamp /var/www/logfiles/Verizon*|grep $CurrentDate|awk -F '\t' '{print $2}'|head -1`;
echo $Latitude;
echo $Longitude;
echo $linecount;
echo $RSSI;
...
echo "L.marker([$Latitude, $Longitude], {icon: $Icon}).addTo(map);" >> /tmp/Verizon-RSSI-daily-tmp.txt;

done;

cat /tmp/Verizon-RSSI-daily-tmp.txt|sort|uniq >> /var/www/Verizon-RSSI-daily-tmp.html;
...
grep -v "{icon: }" /var/www/Verizon-RSSI-daily-tmp.html  >> /var/www/Verizon-RSSI-daily.html
